...
class DynamicallyComposedMultiHeadAttention(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, key_padding_mask=None,
        ...
        # print('\n\n Shape of attn_feature_matrix before compose is: ', attn_feature_matrix.shape, '\n\n')
        uncomposed_attention = attn_feature_matrix.softmax(dim=-1)
        # print('\n\n Shape of uncomposed_attention is: ', uncomposed_attention.shape, '\n\n')
        attn_feature_matrix = self._compose(attn_information=attn_feature_matrix,
                                            query=query, key=key, projection_type='pre')
        # print('\n\n Shape of attn_feature_matrix after compose is: ', attn_feature_matrix.shape, '\n\n')
        # print('\n\n Shape of key_padding_mask after compose is: ', key_padding_mask.shape)
        # print(' Shape of attn_feature_matrix after compose is: ', attn_feature_matrix.shape, '\n\n')
        if key_padding_mask is not None:
            # print('\n\n Shape of key_padding_mask is: ', key_padding_mask.shape, '\n\n')
            key_padding_mask = key_padding_mask.permute(1,0)
            key_padding_mask = key_padding_mask.unsqueeze(1).unsqueeze(2)
            attn_feature_matrix.masked_fill_(key_padding_mask, float(0))
